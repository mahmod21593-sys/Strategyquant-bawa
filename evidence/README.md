...
**Result:** one edge survived every round: **MR-06**. Everything intraday either failed on unseen data,
fell below realistic costs, or (FX) turned out to be a data artifact.

## Edge cards

| Card | Verdict | Grade (literature → own data) | Prop fit | Own-data result |
|---|---|---|---|---|
| **[MR-06 Three down days (new)](edges/MR-06_three_down_days.md)** | **Build first; the only survivor** | — → **A−** | Low–Medium (≈ 40% pass vs 33% base) | +20 bps/trade; confirmed 2013 → ; 15:55 entry works (+15.9 bps, p = 0.03); next-open entry loses half |
...
| [FX-02 Month-end fix hedging](edges/FX-02_month_end_fix_hedging.md) | Untested; needs bid/ask data | B | — | — |
| [CF-02 Rebalancing flows](edges/CF-02_rebalancing_flows.md) | Re-read timing, re-test | B → ? | — | Significant **opposite** sign in my implementation |
...
| [Decay controls](edges/decay_controls.md) | Controls only | D | — | Turn of month, overnight drift and FOMC-day premium all **decayed as predicted**: the pipeline passes calibration |
