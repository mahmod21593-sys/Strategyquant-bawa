...
Every edge below has now also been tested on real data, with the tests pre-registered before any result was seen: [research/validation/REPORT.md](../research/validation/REPORT.md). **Read that report first.** Its verdicts override the literature grades where they disagree.

## Edge cards

| Card | Verdict | Grade | Prop fit | Key change from the original plan |
|---|---|---|---|---|
| **[MR-06 Three down days (new)](edges/MR-06_three_down_days.md)** | **Build first** | A− (own data, confirmed out of sample) | Medium | Found by the pre-registered scan; +21.9 bps/trade 2013 → after costs; US only |
| [IM-01 Market intraday momentum](edges/IM-01_market_intraday_momentum.md) | **Hold**: own data shows the *opposite* sign in 2023–26 | B (was A) | High | Rosa (2022): unconditional version disappears out of sample; Baltussen et al. results are gross of costs |
...
| [MR-01 Index mean reversion](edges/MR-01_index_short_term_reversal.md) | Filter only (IBS not significant on own data; non-US negative); use MR-06 instead | B | High | IBS effect largely **US-only** (Pagonidis); non-US indices must pass on their own |
